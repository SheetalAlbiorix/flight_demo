...
console.log('Flight Details:', flight);
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>{flight.airline} {flight.flightNumber}</Text>
      <Text style={styles.route}>{flight.departure} ➡️ {flight.arrival}</Text>
      <Text style={styles.time}>{flight.time} on {flight.date}</Text>
      <Text style={styles.status}>{flight.status}</Text>
      <Text style={styles.info}>This screen can include maps, gate details, and delay notices from email parsing!</Text>
    </View>
  );
};
